...
const url = 'https://qzaisectszonsdazosup.supabase.co';
const serviceKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InF6YWlzZWN0c3pvbnNkYXpvc3VwIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MzM1ODg5NywiZXhwIjoyMDg4OTM0ODk3fQ.V8TsVd2_nnrprrfzxiSVQnA0vLMahF3nLsnENJQS3Fs';
...
async function run() {
  ...
  console.log('Tables do not exist. Run the following SQL in Supabase SQL Editor:');
...
CREATE POLICY "Users can delete their own tasks" ON tasks FOR DELETE USING (user_id = auth.uid());
  `);
  console.log('='.repeat(60));
  console.log('\nGo to: https://supabase.com/dashboard/project/qzaisectszonsdazosup/sql');
